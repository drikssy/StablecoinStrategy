...
import { InvestableLibBis } from  "./InvestableLibBis.sol";
...
library SwapServiceLib {
    ...
    function swapExactTokensForNative(
        ...
        if (swapService.provider == SwapServiceProvider.BscPancakeSwap) {
            pathNew[pathOldLength] = address(InvestableLibBis.BINANCE_WBNB);
    ...
    function swapExactNativeForTokens(
        ...
        if (swapService.provider == SwapServiceProvider.BscPancakeSwap) {
            pathNew[0] = address(InvestableLibBis.BINANCE_WBNB);
            IUniswapV2LikeRouter router = IUniswapV2LikeRouter(
                swapService.router
            );
