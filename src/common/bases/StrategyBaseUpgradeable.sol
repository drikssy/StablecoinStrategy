    ...
    IStrategy
{
        ...
        nonReentrant
    {
        ...
        checkTotalInvestmentLimit(depositTokenAmountIn, equityValuationBeforeInvestment);
        ...
        investmentToken.mint(
            ...
            InvestableLibBis.calculateMintAmount(
                equityValuationBeforeInvestment, totalEquityChange, investmentTokenSupply, depositToken.decimals()
            )
        );
        ...
        nonReentrant
    {
        ...
        uint256 withdrawnTotalDepositTokenAmount =
            _afterWithdraw(investmentTokenAmountIn, params) - depositTokenBalanceBefore;

        // withdrawing from the uninvested balance
        uint256 withdrawnUninvestedDepositTokenAmount =
            (uninvestedDepositTokenAmount * investmentTokenAmountIn) / investmentToken.totalSupply();
        ...
        uint256 feeDepositTokenAmount =
            (withdrawnTotalDepositTokenAmount * getWithdrawalFee(params)) / Math.SHORT_FIXED_DECIMAL_FACTOR / 100;
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IAum).interfaceId || interfaceId == type(IFee).interfaceId
            || interfaceId == type(IInvestable).interfaceId || interfaceId == type(IReward).interfaceId
            || interfaceId == type(IStrategy).interfaceId || super.supportsInterface(interfaceId);
    }
